...
def fetch_route_names(connection, starting_letter):
    query = f"SELECT DISTINCT route_name FROM bus_routes1 WHERE route_name LIKE '{starting_letter}%' ORDER BY route_name"
    route_names = pd.read_sql(query, connection)['route_name'].tolist()
    return route_names

# Function to fetch data from PostgreSQL based on selected Route_Name and price sort order
def fetch_data(connection, route_name, price_sort_order):
    price_sort_order_sql = "ASC" if price_sort_order == "Low to High" else "DESC"
    query = f"SELECT * FROM bus_routes1 WHERE route_name = %s ORDER BY star_rating DESC, price {price_sort_order_sql}"
